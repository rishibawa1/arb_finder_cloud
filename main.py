...
def best_price_per_team(event_rows: List[Dict[str, Any]], allowed_books: List[str]) -> List[Dict[str, Any]]:
    # if allowed_books is empty or None, do not filter
...
def format_alert(event: str, side1: Dict[str, Any], side2: Dict[str, Any],
                 s1: float, s2: float, profit: float, roi: float) -> str:
    lines = []
...
def run_once(cfg: dict) -> List[Tuple[str, str]]:
    ...
    if cfg.get("use_mock_data", True):
        ...
        records = fetch_the_odds_api(
            ...
            bookmakers=o.get("bookmakers", None),  # list or None
        )
...
def main():
    ...
    # Gather chat ids as a list no matter how config is shaped
    tg = cfg.get("telegram", {})
    bot_token = tg.get("bot_token", "")
    chat_ids = []

    # support either telegram.chat_ids list, or telegram.chat_id list, or a single string
    if isinstance(tg.get("chat_ids"), list):
        chat_ids = [str(c) for c in tg["chat_ids"]]
    elif isinstance(tg.get("chat_id"), list):
        chat_ids = [str(c) for c in tg["chat_id"]]
    elif "chat_id" in tg:
        chat_ids = [str(tg["chat_id"])]

    if not chat_ids:
        print("Warning: no Telegram chat ids configured")
    ...
    if cfg.get("use_mock_data", True):
        ...
        for msg, sig in alerts:
            ...
            print(msg)
            for cid in chat_ids:
                send_message(bot_token, cid, msg)
            cache[sig] = now
